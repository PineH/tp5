<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:65:"D:\xampp\htdocs\tp5\public/../application/admin\view\nav\add.html";i:1508493332;}*/ ?>
<!DOCTYPE html>
<html>
...
<script>
    ...
    $(".btn-info").click(function() {
        ...
        if (title == ''){
            layer.msg('导航标题不能为空！', function(){
            });
            $(".title").focus();
            return false;
        }else if (title.length<2 || title.length>10){
            layer.msg('导航标题长度应在2-10字符之间！',function () {
            });
            $(".title").focus();
            return false;
        }
        $.post(
            "<?php echo url('admin/Nav/add'); ?>",
            {title: title, title_en: title_en, sort: sort, status: status},
            function (data) {
                console.log(data);
                if (data.res == 1) {
                    layer.alert('添加导航成功！', {
